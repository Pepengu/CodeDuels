defmodule CodeDuelsWeb.RoundNotificationPopup do
  use CodeDuelsWeb, :live_component

  def mount(socket) do
    {:ok, assign(socket, show: false, title: "", message: "")}
  end
  ...
  def update(%{action: :show} = assigns, socket) do
    title = assigns[:title] || "Уведомление"
    message = assigns[:message] || ""
    socket = assign(socket, show: true, title: title, message: message)
    {:ok, socket}
  end
  ...
  def update(assigns, socket) do
    {:ok,
     socket
     |> assign(
       title: assigns[:title] || socket.assigns[:title] || "Уведомление",
       message: assigns[:message] || socket.assigns[:message] || ""
     )}
  end
