defmodule CodeDuelsWeb.UserAuthTest do
  use CodeDuelsWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias CodeDuels.Accounts
  alias CodeDuels.Accounts.Scope
  alias CodeDuelsWeb.UserAuth

  import CodeDuels.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CodeDuelsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CodeDuelsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CodeDuelsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
